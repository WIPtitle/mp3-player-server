...
Description: ${DESCRIPTION}
 ...
 and simple HTTP API for integration with other services.
Depends: python3 (>= 3.7), systemd, mpg123
EOF
...
if [ ! -f /etc/audio-server/config.json ]; then
    echo '{"port": 8888, "storage_dir": "/var/lib/audio-server/data"}' > /etc/audio-server/config.json
fi
...
echo '{"port": 8888, "storage_dir": "/var/lib/audio-server/data"}' > ${PACKAGE_DIR}/etc/audio-server/config.json
